 ...
 *
 * <p>
 * <tt>java ec.Evolve -file </tt><i>parameter_file
 * [</i><tt>-p </tt><i>parameter=value]*</i>
 *
 * <p>
...
public class Evolve {
	...
	/**
	 * Restores an EvolutionState from checkpoint if "-checkpoint FILENAME" is in
	...
	/**
	 * Loads a ParameterDatabase from checkpoint if "-params" is in the command-line
	 * arguments.
	 */
			...
			if (args[x].equals(A_AT))
				try {
					if (parameters != null) // uh oh
						Output.initialError(
								"Both -file and -at arguments provided.  This is not permitted.\nFor help, try:  java ec.Evolve -help");
					else
		...
		for (int x = 0; x < args.length - 1; x++)
			if (args[x].equals(A_FROM))
				try {
					if (parameters != null) // uh oh
						Output.initialError(
								"Both -file and -from arguments provided.  This is not permitted.\nFor help, try:  java ec.Evolve -help");
					else {
						if (cls == null) // no -at
							cls = Evolve.class;
					...
					e.printStackTrace();
					Output.initialError("The parameter file is missing at the resource location: " + args[x + 1]
							+ " relative to the class: " + cls + "\n\nFor help, try:  java ec.Evolve -help");
				}

		if (parameters == null)
			Output.initialError(
					"No parameter or checkpoint file was specified.\nFor help, try:   java ec.Evolve -help");
		return parameters;
	}
	...
	public static int determineThreads(Output output, ParameterDatabase parameters, Parameter threadParameter) {
		...
		if (tmp_s == null) // uh oh
		{
			...
			try {
				return ((Integer) runtime.getClass().getMethod("availableProcessors", (Class[]) null).invoke(runtime,
	...
	/**
	 * Primes the generator. Mersenne Twister seeds its first 624 numbers using a
	 * basic linear congruential generator; thereafter it uses the MersenneTwister
	 * algorithm to build new seeds. Those first 624 numbers are generally just
	 * fine, but to be extra safe, you can prime the generator by calling nextInt()
	 * on it some (N>1) * 624 times. This method does exactly that, presently with
	 * N=2.
	 */
	 ...
	 * currentTime value. Then the seed is incremented by the offset. This method is
	 * broken out of initialize(...) primarily to share code with
	 ...
	 * adjustment (added to each random seed). The adjustment offers a convenient
	 * way to change the seeds of the random number generators each time you do a
	 * new evolutionary run. You are of course welcome to replace the random number
	...
	/**
	 * Initializes an evolutionary run given the parameters and a random seed
	 * adjustment (added to each random seed), with the Output pre-constructed. The
	 * adjustment offers a convenient way to change the seeds of the random number
	 * generators each time you do a new evolutionary run. You are of course welcome
	 * to replace the random number generators after initialize(...) but before
	 * startFresh(...)
	 */
	...
	public static EvolutionState initialize(ParameterDatabase parameters, int randomSeedOffset, Output output) {
		...
		for (x = 0; x < random.length; x++) {
			seeds[x] = determineSeed(output, parameters, new Parameter(P_SEED).push("" + x), time + x,
					random.length * randomSeedOffset, auto);
			for (int y = 0; y < x; y++)
				if (seeds[x] == seeds[y])
	 ...
	 * output and checkpoint filenames. If it's null, nothing is added to the front.
	 */
	 ...
	 * MAIN
	 *
	 * Evolve has... evolved from previous Evolves. The goal behind these changes
	 * is: 1. To provide a simple jobs facility 2. To make it easy for you to make
	 * your own main(), including more sophisticated jobs facilities.
	 *
	 * Before we get into the specifics of this file, let's first look at the main
	 * evolution loop in EvolutionState.java. The general code is: 1. If I was
	 * loaded from a checkpoint, call the hook startFromCheckpoint() 2. If I'm
	 * instead starting from scratch, call the hook startFresh() 3. Loop: 4. result
	 * = evolve() 5. If result != EvolutionState.R_NOTDONE, break from loop 6. Call
	 * the hook finish(result)
	 *
	 ...
	 * class, call run(...), and then shut down. The complexity mostly comes from
	 * bringing up the class (loading it from checkpoint or from scratch) and in
	 ...
	 *
	 * - To instead set up from scratch, you have to do a bunch of stuff to set up
	 * the state. First, you need to load a parameter database. Evolve.java has a
	 * convenience function for that called loadParameterDatabase(...). Second, you
	 * must do a series of items: (1) generate an Output object (2) identify the
	 * number of threads (3) create the MersenneTwisterFast random number generators
	 * (4) instantiate the EvolutionState subclass instance (5) plug these items,
	 * plus the random seed offset and the parameter database, into the instance.
	 * These five steps are done for you in a convenience function called
	 * initialize(...).
	 *
	 * - Now the state is ready to go. Call run(...) on your EvolutionState (or do
	 * the evolution loop described above manually if you wish)
	 *
	 * - Finally, to shut down, you need to (1) flush the Output (2) print out the
	 * used, accessed, unused, unaccessed, and all parameters if the user requested
	 * a printout at the end [rarely] (3) flush System.err and System.out for good
	 * measure, and (4) close Output -- which closes its streams except for
	 * System.err and System.out. There is a convenience function for this as well.
	 * It's called cleanup(...).
	 *
	 ...
	 * checkpoint state.run(EvolutionState.C_STARTED_FROM_CHECKPOINT); else { state
	 * = initialize(loadParameterDatabase(args), 0);
	 * state.run(EvolutionState.C_STARTED_FRESH); } cleanup(state); System.exit(0);
	 * }
	 *
	 *
	 * Piece of cake!
	 ...
	 * doing basic job iteration. EvolutionState has two convenience slots for doing
	 * job iteration:
	 *
	 * job (an Object[] use this as you like) runtimeArguments (a String[] put args
	 * in here)
	 *
	 ...
	 *
	 * You'll probably want the EvolutionState to output its stat files etc. using
	 * unique prefixes to differentiate between jobs (0.stat, 1.stat, or whatever
	 * you like -- it doesn't have to be numbers), and you'll also probably want
	 * checkpoint files to be similarly prefixed. So you'll probably want to do:
	 *
	 * state.output.setFilePrefix(jobPrefix); state.checkpointPrefix = jobPrefix +
	 * state.checkpointPrefix;
	 *
	 * The extravagant main below is basically doing this. We're using state.job to
	 * stash away a single iterated job number, stored as an Integer in
	 * state.job[0], and then iterating that way, making sure we stash the job
	 * number and runtime arguments each time so we can recover them when loading
	 * from checkpoint. We use the "jobs" parameter to determine how many jobs to
	 * run. If this number is 1, we don't even bother to set the file prefixes, so
	 * ECJ generates files just like it used to.
	 *
	 * It's important to note that this main was created with the assumption that
	 * you might modify it for your own purposes. Do you want a nested loop, perhaps
	 * to do all combinations of two parameters or something? Rewrite it to use two
	 * array slots in the job array. Want to store more information on a per-job
	 * basis? Feel free to use the job array any way you like -- it's ONLY used by
	 * this main() loop.
	 *
	 */
	...
	public static void main(String[] args) {
		// Upto line 696 is geeksforgeeks code
		...
		log.log(Level.INFO, "This is a log message in Evolve!!!!!");
		...
		// this simple job iterator just uses the 'jobs' parameter, iterating from 0 to
		// 'jobs' - 1
		// inclusive. The current job number is stored in state.jobs[0], so we'll begin
		// there if
		// we had loaded from checkpoint.

		if (state != null) // loaded from checkpoint
		{
			// extract the next job number from state.job[0] (where in this example we'll
			// stash it)
			try {
				if (state.runtimeArguments == null)
					Output.initialError(
							"Checkpoint completed from job started by foreign program (probably GUI).  Exiting...");
		...
		// one such job, we've updated the current job number (currentJob) to the next
		// number.
		// Otherwise currentJob is 0.

		// Now we're going to load the parameter database to see if there are any more
		// jobs.
		// We could have done this using the previous parameter database, but it's no
		// big deal.
		...
		// Now we know how many jobs remain. Let's loop for that many jobs. Each time
		// we'll
		// load the parameter database scratch (except the first time where we reuse the
		// one we
		// just loaded a second ago). The reason we reload from scratch each time is
		// that the
		// experimenter is free to scribble all over the parameter database and it'd be
		// nice to
		// have everything fresh and clean. It doesn't take long to load the database
		// anyway,
		// it's usually small.
		for (int job = currentJob; job < numJobs; job++) {
			// We used to have a try/catch here to catch errors thrown by this job and
			// continue to the next.
			// But the most common error is an OutOfMemoryException, and printing its stack
			// trace would
			// just create another OutOfMemoryException! Which dies anyway and has a
			// worthless stack
			// trace as a result.
			...
			// try
			{
				...
				// Here you can set up the EvolutionState's parameters further before it's
				// setup(...).
				// This includes replacing the random number generators, changing values in
				// state.parameters,
				// changing instance variables (except for job and runtimeArguments, please),
				// etc.

				// now we let it go
				state.output.systemMessage("YYYYEA STARTING!!");
			 ...
			 * e.printStackTrace(); state = null; System.gc(); // take a shot! }
			 */
		}
